<script lang="ts">
  ...
  $: isMandatory = minigame?.mandatory ?? true;
  ...
  function effectSummary(choiceId: string): string {
    ...
    for (const e of choice.effects) {
      ...
      if (e.kind === 'blocSupport')
        parts.push(`${e.delta > 0 ? '+' : ''}${e.delta.toFixed(1)} ${e.groupId.replace(/_/g, ' ')}`);
...
{#if minigame}
  <div class="modal-overlay" role="dialog" aria-modal="true">
    <div class="modal">
      ...
      {#if isMandatory}
        <div class="mandatory-note">This debate is mandatory — the race begins after you respond.</div>
      {/if}
...
<style>
  ...
  .choice-card.selected { border-color: #c8a44a; background: #1e1e30; }
  ...
  .choice-effects {
    ...
    border-top: 1px solid #2a2a3e; padding-top: 4px; margin-top: 2px;
  }
  ...
  .mandatory-note {
    font-size: 0.58rem; color: #555; text-align: center;
